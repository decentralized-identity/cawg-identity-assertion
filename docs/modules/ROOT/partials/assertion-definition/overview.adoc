...
The `entity_id`, `authority_id`, `action`, and `resource` fields of `trust_registry` correspond directly to the required fields of https://trustoverip.github.io/tswg-trust-registry-protocol/approved/#authorization-query-schema[§6.1, Authorization query schema] of the TRQP specification.

`context` _(optional)_ carries additional, ecosystem-specific constraints needed to interpret the authorization, such as a role or region qualifier.
This specification does not currently define a required vocabulary for `context`; a future minor version of this specification MAY define CAWG-specific `context` keys.

`trqp_authorization_uri` _(optional)_ lets the _<<_credential_holder,credential holder>>_ name a specific TRQP-compliant endpoint to query.
It is a CAWG-specific addition and is not itself part of the TRQP query schema, which instead expects the query endpoint to be discovered from `authority_id`, for example via a DID document, as described in https://trustoverip.github.io/tswg-trust-registry-protocol/approved/#authority_id[§5.1, `authority_id`] of the TRQP specification.
If `trqp_authorization_uri` is absent, a validator resolves `authority_id` to either a live query endpoint or a cached _<<_trust_registry_snapshot,trust registry snapshot>>_, as described in xref:_validation_of_trust_registry[xrefstyle=full].
...
NOTE: The procedure for validating a `trust_registry` claim, including both a live TRQP query and a cached _<<_trust_registry_snapshot,trust registry snapshot>>_, is described in xref:_validation_of_trust_registry[xrefstyle=full].
