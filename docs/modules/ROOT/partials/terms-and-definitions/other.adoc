...
==== Archival-quality credential

Any _<<verifiable_identifier,verifiable identifier>>_ that can be used to make claims on behalf of its _<<named_actor,named actor>>_ for which an independent system is able to verify the link between a claim made with that credential and its _<<named_actor,named actor>>_ for the stated validation lifetime of that credential.
Any implementation of an archival-quality credential MUST specify an expected period for which credential validation can be expected to succeed and the means by which any relevant data necessary for verification can be accessed during that stated lifetime.

IMPORTANT: An archival-quality credential can not make guarantees about the ongoing validity of data.
As an example, the legal status of an asset may change due to legal transactions that can't be recorded in an immutable data structure.

==== Assertion claim type

A globally unique, namespaced identifier that names the statement carried by a *<<_named_actor_claim,Named Actor claim>>* (for example, `iptc:vnp`).
In this specification, an _assertion claim type_ appears as a triple `predicate` in `credentialSubject.claims`.
The meaning of an _assertion claim type_ is governed by the issuing *<<_named_actor_claim_authority,Named Actor claim authority>>;* CAWG standardizes only its syntax and does not define the set of possible claim types.

See xref:_named_actor_claim_credential_format[xrefstyle=full].
...
* Does the credential subject need to pre-authorize or pre-specify the means of verification?
...
A signed statement made by a *<<_named_actor_claim_authority,Named Actor claim authority>>* about a _<<_named_actor,named actor>>,_ describing an additional validation, status, or accreditation held by that _<<_named_actor,named actor>>._
Where an *<<_identity_assertion,identity assertion>>* answers _“Who is this entity?”,_ a _Named Actor claim_ answers _“What additional validation, status, or accreditation does this entity have?”_

A _Named Actor claim_ is distinct from the _<<_named_actor,named actor’s>>_ own _<<_verifiable_identifier,verifiable identifier>>_ and from any certificate issuance or certificate lifecycle event.
It is expressed as a CBOR-encoded verifiable credential secured with a COSE signature.

See xref:_named_actor_claims[xrefstyle=full].

==== Named Actor claim authority

An _<<_actor,actor>>_ that issues *<<_named_actor_claim,Named Actor claims>>* about _<<_named_actor,named actors>>._
A _Named Actor claim authority_ governs the vocabulary and semantics of the claims it issues and the policies under which it issues them.
A validator establishes trust in a _Named Actor claim authority_ through its *<<_named_actor_claim_authority_registry,Named Actor claim authority registry>>.*

==== Named Actor claim authority registry

A list, held by a validator, of *<<_named_actor_claim_authority,Named Actor claim authorities>>* together with the public keys used to verify *<<_named_actor_claim,Named Actor claims>>* issued by each.
The _Named Actor claim authority registry_ determines which *<<_named_actor_claim,Named Actor claims>>* a validator is willing to surface.
Its contents are distributed out of band, independently of any individual *<<_named_actor_claim,Named Actor claim>>.*

See xref:_named_actor_claim_trust_model[xrefstyle=full].
