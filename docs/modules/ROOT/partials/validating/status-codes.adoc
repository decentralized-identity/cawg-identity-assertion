...
| The `trust_registry` field is present, and either a live TRQP query or a cached _<<_trust_registry_snapshot,trust registry snapshot>>_ confirmed that `trust_registry.entity_id` is authorized for `trust_registry.action` on `trust_registry.resource` by `trust_registry.authority_id`, as described in xref:_validation_of_trust_registry[xrefstyle=full].
...
| The `trust_registry` field is present, and either a live TRQP query or a cached _<<_trust_registry_snapshot,trust registry snapshot>>_ indicated that `trust_registry.entity_id` is *not* authorized for `trust_registry.action` on `trust_registry.resource` by `trust_registry.authority_id`, as described in xref:_validation_of_trust_registry[xrefstyle=full].

| `cawg.identity.trust_registry.unreachable`
| The `trust_registry` field is present, but neither a live TRQP query nor an unexpired _<<_trust_registry_snapshot,trust registry snapshot>>_ for `trust_registry.authority_id` could be obtained, as described in xref:_validation_of_trust_registry[xrefstyle=full].
