...
Any _<<_verifiable_identifier,verifiable identifier>>_ that can be used to make claims on behalf of its _<<_named_actor,named actor>>_ for which an independent system is able to verify the link between a claim made with that credential and its _<<_named_actor,named actor>>_ with that credential at any time after the claim is made.
