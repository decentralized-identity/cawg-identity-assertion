...
Adapted from link:++https://glossary.trustoverip.org/#term:governance-framework++[Trust Over IP’s definition of governance framework].
...
A repository of additional _<<_c2pa_assertion,C2PA assertions>>_ or additional metadata which can be verified to determine whether the _<<_named_actor,named actor>>_ from the *<<_identity_assertion,identity assertion>>* is associated with the additional information provided.
...
A _trust list_ can be considered a simplified form of a <<_trust_registry,trust registry>>.
...
See also _<<_referenced_registry,referenced registry>>._

Adapted from link:++https://glossary.trustoverip.org/#term:trust-registry++[Trust Over IP’s definition of trust registry].
