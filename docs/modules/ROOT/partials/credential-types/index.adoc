...
An _<<_identity_assertion_consumer,identity assertion consumer>>_ may need to adjust their security posture (e.g. using proxy services when appropriate) to facilitate these requirements. A validator implementation SHOULD use the status code `cawg.identity.network_traffic_blocked` when it is unable to complete validation due to network traffic policy constraints.
