...
*TO DO:* Is this TRQP payload appropriate? If so, we can flesh that out more after the Monday meeting. Look at https://trustoverip.github.io/tswg-trust-registry-protocol/approved/#authorization-query-schema[§6.1, Authorization query schema] to determine what information we need to provide.

*TO DO:* Revisit trust TF meeting from 3 August to see if there is a more appropriate example. Add context parameter based on discussion from that meeting. Context used if there's something specific about the kind of resource (time limits, etc. -- look at TRQP spec). CAWG may add specific vocabulary if useful.
