...
:note-caption: Andrew
NOTE: We'll need to include a validation method for signatures that include no C2PAAssetBinding.
This process should also ensure that any such signatures are enclosed by a VP (which then has to include a C2PAAssetBinding by definition).
...
It can be located as described in link:https://spec.c2pa.org/specifications/specifications/2.3/specs/C2PA_Specification.html#x509_certificates[Section 14.6, “X.509 certificates,”] of the C2PA technical specification.
