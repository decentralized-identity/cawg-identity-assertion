...
For each entry, it SHALL verify each of the conditions stated in xref:#ica-vc-credentialsubject-verifiedIdentities[xrefstyle=full] and issue the failure code `cawg.ica.verified_identities.invalid` if any condition stated there is unmet.
