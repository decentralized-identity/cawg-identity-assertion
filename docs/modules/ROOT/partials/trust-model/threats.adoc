...
This specification does not currently define a mitigation for this class of attack.
Earlier versions of this specification defined optional `signer_payload` fields (`expected_partial_claim`, `expected_claim_generator`, and `expected_countersigners`) that were intended to limit the content of the enclosing _<<C2PA Manifest>>,_ but those fields were never adopted in practice and have since been removed.
A future revision of this specification may reintroduce a mechanism to address this threat.
