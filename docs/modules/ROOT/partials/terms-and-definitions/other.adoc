...
A globally unique, namespaced identifier that names the statement carried by a *<<_signer_claim,signer claim>>* (for example, `iptc:vnp`).
The meaning of an _assertion claim type_ is governed by the issuing *<<_signer_claim_authority,signer claim authority>>;* CAWG standardizes only its syntax and does not define the set of possible claim types.

See xref:_signer_claim_credential_format[xrefstyle=full].
...
A signed statement made by a *<<_signer_claim_authority,signer claim authority>>* about a _<<_named_actor,named actor>>,_ describing an additional validation, status, or accreditation held by that _<<_named_actor,named actor>>._
Where an *<<_identity_assertion,identity assertion>>* answers _“Who is this entity?”,_ a _signer claim_ answers _“What additional validation, status, or accreditation does this entity have?”_

A _signer claim_ is distinct from the _<<_named_actor,named actor’s>>_ own _<<_verifiable_identifier,verifiable identifier>>_ and from any certificate issuance or certificate lifecycle event.
It is expressed as a CBOR Web Token secured with a COSE signature.

See xref:_signer_claims[xrefstyle=full].

==== Signer claim authority

An _<<_actor,actor>>_ that issues *<<_signer_claim,signer claims>>* about _<<_named_actor,named actors>>._
A _signer claim authority_ governs the vocabulary and semantics of the claims it issues and the policies under which it issues them.
A validator establishes trust in a _signer claim authority_ through its *<<_signer_claim_authority_registry,signer claim authority registry>>.*

==== Signer claim authority registry

A list, held by a validator, of *<<_signer_claim_authority,signer claim authorities>>* together with the public keys used to verify *<<_signer_claim,signer claims>>* issued by each.
The _signer claim authority registry_ determines which *<<_signer_claim,signer claims>>* a validator is willing to surface.
Its contents are distributed out of band, independently of any individual *<<_signer_claim,signer claim>>.*

See xref:_signer_claim_trust_model[xrefstyle=full].
