...
A globally unique, namespaced identifier that names the statement carried by a *<<_trusted_assertion,trusted assertion>>* (for example, `iptc:vnp`).
The meaning of an _assertion claim type_ is governed by the issuing *<<_trusted_assertion_authority,trusted assertion authority>>;* CAWG standardizes only its syntax and does not define the set of possible claim types.

See xref:_trusted_assertion_credential_format[xrefstyle=full].
...
A signed statement made by a *<<_trusted_assertion_authority,trusted assertion authority>>* about a _<<_named_actor,named actor>>,_ describing an additional validation, status, or accreditation held by that _<<_named_actor,named actor>>._
Where an *<<_identity_assertion,identity assertion>>* answers _“Who is this entity?”,_ a _trusted assertion_ answers _“What additional validation, status, or accreditation does this entity have?”_

A _trusted assertion_ is distinct from the _<<_named_actor,named actor’s>>_ own _<<_verifiable_identifier,verifiable identifier>>_ and from any certificate issuance or certificate lifecycle event.
It is expressed as a CBOR Web Token secured with a COSE signature.

See xref:_trusted_assertions[xrefstyle=full].

==== Trusted assertion authority

An _<<_actor,actor>>_ that issues *<<_trusted_assertion,trusted assertions>>* about _<<_named_actor,named actors>>._
A _trusted assertion authority_ governs the vocabulary and semantics of the claims it issues and the policies under which it issues them.
A validator establishes trust in a _trusted assertion authority_ through its *<<_trusted_assertion_authority_registry,trusted assertion authority registry>>.*

==== Trusted assertion authority registry

A list, held by a validator, of *<<_trusted_assertion_authority,trusted assertion authorities>>* together with the public keys used to verify *<<_trusted_assertion,trusted assertions>>* issued by each.
The _trusted assertion authority registry_ determines which *<<_trusted_assertion,trusted assertions>>* a validator is willing to surface.
Its contents are distributed out of band, independently of any individual *<<_trusted_assertion,trusted assertion>>.*

See xref:_trusted_assertion_trust_model[xrefstyle=full].
